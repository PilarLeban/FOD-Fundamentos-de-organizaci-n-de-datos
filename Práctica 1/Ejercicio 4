...
begin
    leer(e);
    encontre := false;
    writeln('Ingrese el número de empleado y luego la edad que le quiera modificar: ');
    readln(num);
    readln(edadNueva);
    while (not EOF(arc) and not encontre) do begin 
        ...
        if (e.numero = num) then begin
            encontre := true;
            e.edad := edadNueva;
            seek(arc, filePos(arc)-1); // RETROCEDEMOS PORQUE EL READ NOS HIZO AVANZAR A OTRA POSICIÓN (1 MÁS)
            write(arc, e);
        end;
    end;
end;    
...
{exportar a un archivo de texto llamado: faltaDNIEmpleado.txt, 
los empleados que no tengan cargado el DNI');}
